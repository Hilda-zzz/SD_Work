...
void DebugDrawRingHighRes(float thickness, float innerRadius, Rgba8 color, Vec2 ori, int segments)
{
	int vertexCount = segments * 6;
	std::vector<Vertex_PCU> vertices(vertexCount);

	float angleStep = 360.f / (float)segments;

	for (int j = 0; j < segments; ++j)
	{
		float angle0 = angleStep * (float)j;
		float angle1 = angleStep * (float)(j + 1);

		Vec2 innerFirst = Vec2::MakeFromPolarDegrees(angle0, innerRadius) + ori;
		Vec2 outerFirst = Vec2::MakeFromPolarDegrees(angle0, innerRadius + thickness) + ori;
		Vec2 innerSecond = Vec2::MakeFromPolarDegrees(angle1, innerRadius) + ori;
		Vec2 outerSecond = Vec2::MakeFromPolarDegrees(angle1, innerRadius + thickness) + ori;

		int i = j * 6;
		vertices[i + 0] = Vertex_PCU(Vec3(innerFirst.x, innerFirst.y, 0.f), color, Vec2::ZERO);
		vertices[i + 1] = Vertex_PCU(Vec3(outerFirst.x, outerFirst.y, 0.f), color, Vec2::ZERO);
		vertices[i + 2] = Vertex_PCU(Vec3(outerSecond.x, outerSecond.y, 0.f), color, Vec2::ZERO);
		vertices[i + 3] = Vertex_PCU(Vec3(innerFirst.x, innerFirst.y, 0.f), color, Vec2::ZERO);
		vertices[i + 4] = Vertex_PCU(Vec3(outerSecond.x, outerSecond.y, 0.f), color, Vec2::ZERO);
		vertices[i + 5] = Vertex_PCU(Vec3(innerSecond.x, innerSecond.y, 0.f), color, Vec2::ZERO);
	}

	g_theRenderer->DrawVertexArray(vertexCount, vertices.data());
}
