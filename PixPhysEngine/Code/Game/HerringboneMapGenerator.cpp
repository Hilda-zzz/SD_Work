...
//}
//
//void HerringboneMapGenerator::RenderPixelsToBuffer(std::vector<std::vector<Rgba8>>& outPixels, IntVec2 const& bufferSize, IntVec2 const& sourceOffset) const
//{
//	if (bufferSize.x <= 0 || bufferSize.y <= 0) 
//	{
//		DebuggerPrintf("ERROR: Invalid buffer size (%d, %d)\n", bufferSize.x, bufferSize.y);
//		return;
//	}
//
//	if (static_cast<int>(outPixels.size()) != bufferSize.y) 
//	{
//		DebuggerPrintf("ERROR: Buffer height mismatch - expected %d, got %d\n",
//			bufferSize.y, static_cast<int>(outPixels.size()));
//		return;
//	}
//
//	DebuggerPrintf("Rendering pixels to buffer: size=%dx%d, offset=(%d,%d)\n",
//		bufferSize.x, bufferSize.y, sourceOffset.x, sourceOffset.y);
//
//	for (int bufferY = 0; bufferY < bufferSize.y; ++bufferY) 
//	{
//		// 确保该行已分配
//		if (static_cast<int>(outPixels[bufferY].size()) != bufferSize.x) {
//			DebuggerPrintf("ERROR: Buffer row %d width mismatch\n", bufferY);
//			continue;
//		}
//
//		for (int bufferX = 0; bufferX < bufferSize.x; ++bufferX) 
//		{
//			// 计算在生成区域中的源坐标
//			IntVec2 sourceHPixel(bufferX + sourceOffset.x, bufferY + sourceOffset.y);
//
//			// 检查源坐标是否在生成区域内
//			if (sourceHPixel.x < 0 || sourceHPixel.x >= m_regionSizeHPixels.x ||
//				sourceHPixel.y < 0 || sourceHPixel.y >= m_regionSizeHPixels.y) {
//				// 超出范围，填充默认颜色
//				outPixels[bufferY][bufferX] = Rgba8::BLACK;
//				continue;
//			}
//
//			// 查找该位置对应的tile
...
//					);
//
//					outPixels[bufferY][bufferX] = pixelColor;
//					found = true;
//					break;
//				}
//			}
...
//			// 如果没找到对应的tile，填充默认颜色
//			if (!found) {
//				outPixels[bufferY][bufferX] = Rgba8::MAGNETA;  // 调试：用洋红色标记未覆盖区域
//			}
//		}
//	}
//
//	DebuggerPrintf("Render to buffer complete\n");
//}
//
