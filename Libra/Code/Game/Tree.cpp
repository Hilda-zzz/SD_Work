...
Tree::Tree(EntityType type, EntityFaction faction) :Entity(type, faction)
{
	...
	AddVertsForOBB2D(m_verts_body, m_bodyBox, Rgba8(255, 255, 255, 255), g_decoSpriteSheet->GetSpriteUVs(4).m_mins, g_decoSpriteSheet->GetSpriteUVs(4).m_maxs);
	m_texTree = &g_decoSpriteSheet->GetTexture();
...
void Tree::Render() const
{
	...
	g_theRenderer->BindTexture(m_texTree);
