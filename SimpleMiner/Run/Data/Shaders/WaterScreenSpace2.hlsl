...
float NDCDepthToViewDepth(float ndcDepth, float near, float far)
{
    ...
    return (near * far) / (near - ndcDepth * (near - far));
}
...
float4 PixelMain(v2p_t input) : SV_Target0
{
    ...
    float waterViewDepth = input.clipPosition.w;
    float underwaterDepth = pixelViewDepth - waterViewDepth;
    underwaterDepth = max(0.0, underwaterDepth);
